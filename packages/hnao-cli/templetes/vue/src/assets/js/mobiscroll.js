...
*/ ;(function (
  ...
  function Scroller (elem, settings) {
    ...
    function position (check) {
      if (
        s.display == 'inline' ||
        (ww === $(window).width() && rwh === $(window).height() && check)
      ) {
        return
      } // 修改了下面一行的wh=rwh,原来是wh=wh||rwh
      var w,
        l,
        t,
        aw,
      ...
      if (s.tap) {
        el
          ...
          .bind('touchend', function (e) {
            if (
              Math.abs(getCoord(e, 'X') - startX) < 20 &&
              Math.abs(getCoord(e, 'Y') - startY) < 20
            ) {
              handler.call(this, e)
            }
            tap = true
    ...
    that.show = function (prevAnim) {
      ...
      for (i = 0; i < s.wheels.length; i++) {
        ...
        for (label in s.wheels[i]) {
          ...
          html +=
            '</div><div class="dwwo"></div></div><div class="dwwol"></div></div></td>'
          l++
        }
      ...
      html +=
        (s.display != 'inline'
          ? '<div class="dwbc' +
            ...
            '</span></span>' +
            (s.button3
              ? '<span class="dwbw dwb-n"><span class="dwb">' +
                s.button3Text +
                '</span></span>'
              : '') +
      ...
      dw
        .delegate('.dwwl', 'DOMMouseScroll mousewheel', function (e) {
          if (!isReadOnly(this)) {
            e.preventDefault()
            e = e.originalEvent
  ...
  function getCoord (e, c) {
    var org = e.originalEvent,
      ct = e.changedTouches
    return ct || (org && org.changedTouches)
      ? org
        ? org.changedTouches[0]['page' + c]
        : ct[0]['page' + c]
      : e['page' + c]
  }
  function bool (v) {
    ...
    onMove = function (e) {
      if (scrollable) {
        ...
        inst.scroll(
          target,
          index,
          constrain(pos + (start - stop) / h, min - 1, max + 1)
        )
      }
      moved = true
    },
