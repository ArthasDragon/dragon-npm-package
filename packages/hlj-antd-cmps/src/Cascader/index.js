...
export default class extends PureComponent {
  ...
  render() {
    const {
      extraOptions, extraText, showExtra, placement,
      overlayStyle, value, selectedColor, ...rest
    } = this.props
    let extraContent = []
    if (showExtra) {
      extraContent = extraOptions.map(item => {
        let _value = Array.isArray(value) ? value : []
        let flag = _value.length === 2
          ? item.value[1] === _value[1] : item.value[0] === _value[0]
        return (
          <span
            key={item.key}
            onClick={this.selectCity(item.value, item.label)}
            style={{margin: '2px 2px 4px 2px', display: 'inline-block'}}
          >
              <Tag color={flag ? selectedColor : ''}>
                  {item.label}
              </Tag>
          </span>
        )
